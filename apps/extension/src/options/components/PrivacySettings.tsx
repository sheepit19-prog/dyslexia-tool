...
export const PrivacySettings: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="mb-2 text-xl font-semibold text-gray-900">Privacy &amp; Data</h2>
        <p className="text-sm text-gray-500">Manage your data, export it, or delete everything.</p>
      </div>

      {/* Storage Usage */}
      <section>
        <h3 className="mb-3 text-sm font-medium text-gray-700">Storage Usage</h3>
        {loading ? (
          <p className="text-sm text-gray-500">Loading storage stats...</p>
        ) : stats ? (
          <div className="space-y-2 rounded-lg border border-gray-200 bg-white p-4">
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Notes</span>
              <span className="font-medium text-gray-900">{stats.notes} item{stats.notes !== 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Site Preferences</span>
              <span className="font-medium text-gray-900">{stats.sitePreferences} item{stats.sitePreferences !== 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Analytics entries</span>
              <span className="font-medium text-gray-900">{stats.analytics} item{stats.analytics !== 1 ? 's' : ''}</span>
            </div>
          </div>
        ) : null}
      </section>
      ...
      {/* Export */}
      <section>
        <h3 className="mb-3 text-sm font-medium text-gray-700">Export Data</h3>
      ...
      {/* Nuclear Delete */}
      <section>
        <h3 className="mb-3 text-sm font-medium text-gray-700">Delete All Data</h3>
        <p className="mb-3 text-sm text-gray-500">
        ...
        {!showDelete ? (
          ...
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 space-y-3">
            ...
            <input
              ...
              className="w-full max-w-xs rounded-md border border-red-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
              autoComplete="off"
            />
            {/* Live match progress */}
